...
Dashboard sign-in is a local password or optional OpenID Connect (Authentik). OIDC settings live on `platform_settings`. The authorization-code callback is `/api/auth/oidc/callback`. JIT, when enabled, creates a local user and a platform-tenant membership. Forgot password uses hashed one-hour tokens and the platform sender from Configuration → Basics.
