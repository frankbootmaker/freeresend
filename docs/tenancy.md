...
**OIDC** (Portal → Configuration) signs dashboard users in through Authentik or another OpenID Connect provider. The IdP `email` claim maps to `users.email`. An optional button label replaces the default “Continue with Authentik” text on Sign in. With JIT on, a first sign-in creates the user and a `platform` tenant membership; an optional group can set `is_platform_admin`. With JIT off, only existing local accounts can complete sign-in.
