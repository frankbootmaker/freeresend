...
Optional Authentik / OIDC is configured later in Portal → **Configuration → OIDC** (issuer, client ID/secret, optional sign-in button label, JIT). The platform sender for password-reset mail is **Configuration → System domain** (`PLATFORM_FROM` as env fallback). See `.env.local.example`.
